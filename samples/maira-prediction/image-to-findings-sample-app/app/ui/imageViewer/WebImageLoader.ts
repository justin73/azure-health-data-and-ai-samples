...
/**
 * Cornerstone image loader to support JPG and PNG images.  This implementation
 * is derived from Cornerstone example webloader.
 * 
 * @see https://www.cornerstonejs.org/live-examples/webloader
 */
const WebImageLoader = (() => {
   ...
   */
  function createImage(image: any, imageId: any) {
    ...
    function isImageDrawn() {
      return lastImageIdDrawn === imageId;
    }

    function getCanvasContext() {
      const context = canvas.getContext('2d');
      if (!context) throw new Error("Unable to get context from canvas");
      return context;
    }
    ...
    function getImageData() {
      ...
      if (!isImageDrawn()) drawImage(context);
      return context.getImageData(0, 0, image.naturalWidth, image.naturalHeight);
    }

    function getCanvas() {
      if (isImageDrawn()) return canvas;
      drawImage(getCanvasContext());
      return canvas;
    }
  ...
  ): { promise: Promise<any>; cancelFn: () => void } {
